...
export const defaultInvestmentData: InvestmentData = {
  ...
  interestRate: 6.5,
  loanTerm: 30,
  units: Array(10).fill({ rent: 1000, otherIncome: 0, occupied: true }),
...
export function useInvestmentCalculations(data: InvestmentData) {
  return useMemo(() => {
    ...
    // Operating Expenses
    const monthlyOpEx = (data.taxes || 0) + 
      (data.insurance || 0) + 
      (data.utilities || 0) + 
      (data.trashSnow || 0) + 
      (data.landscaping || 0) + 
    ...
    if (coc !== null && dscr !== null) {
      if (coc >= data.excellentCoc && dscr >= data.excellentDscr) {
        investmentGrade = "Excellent";
      } else if (coc >= data.minCoc && dscr >= data.minDscr) {
    ...
    if (stressedDscr !== null) {
      if (stressedDscr >= data.excellentStressDscr) {
        stressGrade = "Excellent";
      } else if (stressedDscr >= data.minStressDscr) {
        stressGrade = "Good";
      } else {
        stressGrade = "Poor";
      }
